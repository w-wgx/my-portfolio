...
const Hyperspeed = forwardRef<unknown, HyperspeedProps>(({ effectOptions = DEFAULT_EFFECT_OPTIONS }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    speedUp: () => {
      if (appRef.current) {
        appRef.current.speedUpExternal();
      }
    },
    slowDown: () => {
      if (appRef.current) {
        appRef.current.slowDownExternal();
      }
    }
  }));
  ...
  useEffect(() => {
    ...
    class App {
      constructor(container, options = {}) {
        ...
        this.speedUpExternal = this.speedUpExternal.bind(this);
        this.slowDownExternal = this.slowDownExternal.bind(this);
      ...
      initPasses() {
        // No post-processing passes needed for compatibility
        // Using simple WebGL rendering
      }

      loadAssets() {
        // No assets to load without postprocessing
      ...
      render(delta) {
        // Direct renderer rendering instead of composer
